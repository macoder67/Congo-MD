...
global.channelLink = "https://whatsapp.com/channel/0029Vb5kA7k4IBhNW3Hhxy0s";
global.ytch = "@Ontiretsemanyaka";
...
async function handleMessages(sock, messageUpdate, printLog) {
    try {
        ...
        if (userMessage.startsWith('.')) {
            console.log(`📝 Command used in ${isGroup ? 'group' : 'public'}: ${userMessage}`);
        }
        ...
        if (isBanned(senderId) && !userMessage.startsWith('.unban')) {
            ...
            if (Math.random() < 0.1) { 
                await sock.sendMessage(chatId, { 
                    text: '❌ You are banned from using the Congo-MD. Contact an admin to get unbanned.',
                    ...channelInfo
                });
            }
            return;
        ...
        if (!isGroup && (userMessage === 'hi' || userMessage === 'hello' || userMessage === 'bot' || userMessage === 'how are You' || userMessage === 'hey' || userMessage === 'bro')) {
            await sock.sendMessage(chatId, {
                text: 'Hi, How can Congo-MD help you?\nYou can use .menu/.help for more info and commands.',
                ...channelInfo
            });
            return;
        }
        ...
        if (isGroup && isAdminCommand) {
            ...
            if (!isBotAdmin) {
                await sock.sendMessage(chatId, { text: 'Please make me an admin to in order to commerce admin commands.', ...channelInfo });
                return;
            }
